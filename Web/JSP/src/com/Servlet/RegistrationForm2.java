package com.Servlet;

import java.io.IOException;
import java.io.PrintWriter;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;



public class RegistrationForm2 extends HttpServlet 
{
	public void doGet(HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException 
	{
		PrintWriter pw = res.getWriter();
		String result = "";
		boolean flag=true;
		String regx = "\\d+";
		res.setContentType("text/html");
		pw.println("This is servlet page.<br>");
		if(req.getParameter("txtFirstName") == "")
		{
			result += "First Name not entered<br>";
			flag=false;
		}
		else if(regx.matches(req.getParameter("txtFirstName")))
		{
			result += "First Name can have only characters<br>";
			flag=false;
		}
		
		if(req.getParameter("txtLastName")== "") 
		{
			result += "Last Name not entered<br>";
		    flag=false;
		}
		else if(regx.matches(req.getParameter("txtLastName")))
		{
			result += "Last Name can have only characters<br>";
			flag=false;
		}
		
		if(req.getParameter("password")== "") 
		{
			result += "Please specify password<br>";
		    flag=false;
		}
		else if(req.getParameter("password").length() < 8 )
		{
			result += "Password Length should be greater than 8 <br>";
		     flag=false;
		}
		
		if(req.getParameter("ConfirmPassword")== "") 
		{
			result += "Please specify confirm password<br>";
		    flag=false;
		}
		else if(!req.getParameter("password").equalsIgnoreCase(req.getParameter("ConfirmPassword"))) 
		{  
			result += "Password doesn't match<br>";
			flag=false;
		}
		
	
	   if( req.getParameter("txtAge")=="")
		{
		   result += "Age is not entered<br>";
		   flag=false;	
		}
	   else if(!req.getParameter("txtAge").matches("[0-9]+"))
	   {
		   result += "Age can have only numbers<br>";
		   flag=false;
	   }
	   
		if(flag==true)
		{
			result += "User created successfully <br>Form is validated at server side<br>";
		}
		
		req.setAttribute("result", result);
		RequestDispatcher rd = req.getRequestDispatcher("ValidationCheck");
		rd.forward(req, res);
	}
}	
